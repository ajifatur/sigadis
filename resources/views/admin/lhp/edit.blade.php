...
@section('title', 'Edit Laporan Hasil Pemeriksaan')

@section('content')

<div class="d-sm-flex justify-content-between align-items-center mb-3">
    <h1 class="h3 mb-0">Edit Laporan Hasil Pemeriksaan</h1>
</div>
<div class="row">
	<div class="col-12">
        <div class="card">
            <div class="card-body">
                <form method="post" action="{{ route('admin.lhp.store') }}" enctype="multipart/form-data">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <div class="input-group input-group-sm">
                                <input type="text" name="tanggal_surat" class="form-control form-control-sm {{ $errors->has('tanggal_surat') ? 'border-danger' : '' }}" value="{{ date('d/m/Y', strtotime($lhp->tanggal_surat)) }}" autocomplete="off">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <input type="text" name="tempat_surat" class="form-control form-control-sm {{ $errors->has('tempat_surat') ? 'border-danger' : '' }}" value="{{ $lhp->tempat_surat }}">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <input type="text" name="tempat_penerima_surat" class="form-control form-control-sm {{ $errors->has('tempat_penerima_surat') ? 'border-danger' : '' }}" value="{{ $lhp->tempat_penerima_surat }}">
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <select name="hukdis" class="form-select form-select-sm">
                                <option value="" disabled selected>--Pilih--</option>
                                @foreach($hukdis as $h)
                                <option value="{{ $h->id }}" {{ $lhp->hukdis_id == $h->id ? 'selected' : '' }}>{{ $h->jenis->nama }} - {{ $h->nama }}</option>
                                @endforeach
                            </select>
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-lg-10 col-md-9">
                            <select name="tembusan[]" class="form-select form-select-sm {{ $errors->has('tembusan') ? 'border-danger' : '' }}" data-url="{{ route('api.tembusan.store') }}" data-token="{{ csrf_token() }}" multiple>
                                ...
                                <option value="{{ $t->id }}" {{ in_array($t->id, \App\Models\TembusanSurat::where('table_id','=',$lhp->id)->where('table_name','=','tbl_lhp')->pluck('tembusan_id')->toArray()) ? 'selected' : '' }}>{{ $t->name }}</option>
                                @endforeach
                            </select>
    ...
    $(window).on("load", function() {
        $.ajax({
            ...
            success: function(response) {
                ...
                $("select[name=penerima_surat]").val("{{ $lhp->penerima_surat }}");
            }
        });
