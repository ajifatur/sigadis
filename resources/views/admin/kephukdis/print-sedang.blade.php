...
<body>
    ...
    <table class="tabel-form">
        <tr>
            ...
            <td width="5">:</td>
            <td width="5">a.</td>
            <td width="100%">bahwa berdasarkan hasil pemeriksaan Sdr. {{ $kephukdis->terlapor_json->nama }}, NIP {{ $kephukdis->terlapor_json->nip }}, telah terbukti melakukan perbuatan berupa {{ $kephukdis->dugaan_pelanggaran }};</td>
        </tr>
        <tr>
            <td></td>
            <td></td>
    ...
    <table class="tabel-form">
        ...
        <tr>
            <td>KEDUA</td>
            <td>:</td>
            <td>Terhitung mulai tanggal {{ date('j', strtotime($kephukdis->tmt)) }}, bulan {{ \Ajifatur\Helpers\DateTimeExt::month(date('m', strtotime($kephukdis->tmt))) }}, tahun {{ date('Y', strtotime($kephukdis->tmt)) }}, tunjangan kinerja Sdr. {{ $kephukdis->terlapor_json->nama }}, dipotong sebesar Rp. {{ number_format($kephukdis->tukin,0,'.','.') }}, menjadi Rp. {{ number_format($kephukdis->tukin * 0.75,0,'.','.') }}, dan terhitung mulai tanggal {{ date('j', strtotime($kephukdis->tmt_pengembalian)) }}, bulan {{ \Ajifatur\Helpers\DateTimeExt::month(date('m', strtotime($kephukdis->tmt_pengembalian))) }}, tahun {{ date('Y', strtotime($kephukdis->tmt_pengembalian)) }}, tunjangan kinerjanya dikembalikan pada tunjangan kinerja semula.</td>
        </tr>
        <tr>
            <td>KETIGA</td>
            <td>:</td>
