...
<body>
    ...
    <table class="tabel-form">
        <tr>
            ...
            <td width="5">:</td>
            <td width="5">a.</td>
            <td width="100%">bahwa berdasarkan hasil pemeriksaan tim pemeriksa, Sdr. {{ $kephukdis->terlapor_json->nama }}, NIP {{ $kephukdis->terlapor_json->nip }}, telah terbukti melakukan perbuatan berupa {{ $kephukdis->dugaan_pelanggaran }};</td>
        </tr>
        <tr>
            <td></td>
            <td></td>
    ...
    <table class="tabel-form">
        ...
        <tr>
            <td>KEDUA</td>
            <td>:</td>
            <td>Terhitung mulai tanggal {{ date('j', strtotime($kephukdis->tmt)) }}, bulan {{ \Ajifatur\Helpers\DateTimeExt::month(date('m', strtotime($kephukdis->tmt))) }}, tahun {{ date('Y', strtotime($kephukdis->tmt)) }}, Sdr. {{ $kephukdis->terlapor_json->nama }} yang semula menduduki jabatan {{ $kephukdis->terlapor_json->jabatan }} diturunkan menjadi jabatan {{ $kephukdis->jabatan_setelah_diturunkan }}.</td>
        </tr>
        <tr>
            <td>KETIGA</td>
            <td>:</td>
            <td>Atas penurunan jabatan tersebut, hak-hak kepegawaian dari Sdr. {{ $kephukdis->terlapor_json->nama }} disesuaikan dengan jabatan terbaru.</td>
        </tr>
        <tr>
            <td>KEEMPAT</td>
            <td>:</td>
            <td>Pengangkatan dalam jabatan yang baru dalam rangka penurunan jabatan setingkat lebih rendah, ditetapkan dengan keputusan tersendiri sesuai ketentuan peraturan perundang-undangan.</td>
        </tr>
        <tr>
            <td>KELIMA</td>
            <td>:</td>
